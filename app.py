...
# Streamlit Cloud filesystem is ephemeral. Keep the active
# history in session_state and use the local CSV only as an
# optional best-effort cache for local/temporary environments.
# =========================================================
SIGNAL_HISTORY_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "signal_history.csv"
)
HISTORY_STATE_KEY = "_risal_signal_history_v72"

def _safe_read_history_file():
...
def _safe_write_history_file(df):
    try:
        # Best effort only. A Streamlit Cloud restart can discard this file,
        # but a write failure must never crash the dashboard.
...
def load_signal_history():
    if HISTORY_STATE_KEY not in st.session_state:
        st.session_state[HISTORY_STATE_KEY] = _safe_read_history_file()

    df = st.session_state[HISTORY_STATE_KEY]
    if not isinstance(df, pd.DataFrame):
        df = pd.DataFrame()
        st.session_state[HISTORY_STATE_KEY] = df
...
def append_signal_rows(rows):
    ...
    combined = new_df if existing.empty else pd.concat(
    ...
    if {"Detected At", "Symbol", "Signal"}.issubset(combined.columns):
        combined = combined.drop_duplicates(
            subset=["Detected At", "Symbol", "Signal"],
            keep="last"
        ).reset_index(drop=True)

    st.session_state[HISTORY_STATE_KEY] = combined
    _safe_write_history_file(combined)
...
with st.sidebar:
    ...
    st.divider()
    st.caption("📁 Signal history: session storage + optional local CSV")
    st.caption("⚠️ Tracking runs while dashboard is running.")

run_every = f"{refresh_interval}s" if live_enabled else None

# Streamlit Cloud should install a modern version via requirements.txt.
# This fallback prevents a hard decorator error if an older runtime is used.
...
def live_dashboard():
    ...
    st.caption("শুধু A-Level এবং সবচেয়ে শক্তিশালী Top 3 Small-Cap Leader snapshots tracking history-তে সংরক্ষণ হবে।")
    ...
    if history_df.empty:
        ...
        history_column_config = {"_History ID": None}
        checkbox_column_cls = getattr(
            getattr(st, "column_config", None),
            "CheckboxColumn",
            None
        )
        if checkbox_column_cls is not None:
            history_column_config["Delete"] = checkbox_column_cls(
                "🗑️ Delete",
        ...
        if delete_selected:
            ...
            if not selected_ids:
                ...
                st.session_state[HISTORY_STATE_KEY] = history_df
                _safe_write_history_file(history_df)
    ...
    for item in [
        ...
        "Higher-low / compression detection", "Session-based signal tracking"
    ]:
        st.write("✅ " + item)
